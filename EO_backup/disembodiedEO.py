...
def incrementNeuron(neuronName, weight):
    ...
    if psn[nextState] > threshold and not (
    psn[2]):  # 2 represents the boolean column for if neuron already appended
...
def motorcontrol():
    ...
    for pscheck in postsynaptic:
        if pscheck in musDleft or pscheck in musVleft:
            ...
            postsynaptic[pscheck][
                thisState] = 0  # Only thisState weight is reset to 0, because nextState weight
